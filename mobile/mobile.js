...
(function () {
  ...
  function renderCalm(){
    ...
    return `<section>${pageHead("Feelings & Calm","Name a feeling, breathe, or gently pop bubbles.")}
      <div class="mobile-feelings">${feelings.map(([name,icon,detail])=>{const phrase=`I am ${name.toLocaleLowerCase()}`;return `<button class="mobile-feeling" type="button" data-feeling="${esc(name)}|${esc(detail)}" data-feeling-voice="${esc(phrase)}" data-feeling-card-id="${esc(BB_COMMUNICATION_CARDS.cardId("Feelings",phrase))}"><span>${icon}</span>${esc(name)}</button>`;}).join("")}</div>
